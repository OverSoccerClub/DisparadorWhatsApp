// Sistema inteligente de variações automáticas de mensagens
...
export function generateMessageVariations(message: string, count: number): string[] {
  if (count <= 1) return [message]
  
  const variations: string[] = [message] // Primeira variação é a original
...
function applySynonymSubstitutions(message: string, variationIndex: number): string {
  let variation = message
...
function applyMoreSynonymSubstitutions(message: string, variationIndex: number): string {
  let variation = message
...
export function detectMessageType(message: string): 'promocional' | 'informativa' | 'pessoal' | 'comercial' {
...
export function generateTypedVariations(message: string, count: number): string[] {
