...
exports.ConversationAgentService = void 0;
...
class ConversationAgentService {
     ...
     */
    static async generateContextualMessage(request) {
        ...
        const recentHistory = previousMessages.slice(-3).map((msg, idx) => `Mensagem ${idx + 1}: ${msg}`).join('\n');
        let prompt = '';
        switch (messageType) {
            case 'greeting':
...
Gere APENAS a mensagem, sem explicações ou comentários adicionais.`;
     ...
     */
    static getFallbackMessage(messageType, to) {
        const greetings = [
            `Oi! Tudo bem?`,
...
exports.ConversationAgentService = ConversationAgentService;
ConversationAgentService.genAI = null;
