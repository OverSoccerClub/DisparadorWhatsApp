...
exports.WahaVariationService = void 0;
const generative_ai_1 = require("@google/generative-ai");
class WahaVariationService {
    static initializeAI() {
    ...
    static parseVariations(text, expectedCount, originalMessage) {
        ...
        const normalize = (s) => s
            ...
            .replace(/[^a-z0-9\s]/g, ' ');
        const splitWords = (s) => normalize(s)
            .split(/\s+/)
        ...
        const originalWords = originalMessage ? new Set(splitWords(originalMessage)) : new Set();
        const isHeadingOrGroup = (s) => /^(#{1,6}\s|[-*]\s|\d+\.|grupo\s*\d+[:]?|secao\s*\d+[:]?|varia(c|ç)ao\s*\d+[:]?|link\s+para\s+usar\s+em\s+todas\s+as\s+mensagens)/i.test(s);
        for (const line of lines) {
            ...
            if (isIntro || isHeadingOrGroup(cleaned))
                continue;
    ...
    static validateMessageForVariations(message) {
        const issues = [];
        const suggestions = [];
...
exports.WahaVariationService = WahaVariationService;
WahaVariationService.genAI = null;
